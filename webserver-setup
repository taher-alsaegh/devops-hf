Erstellen der Infrastruktur ohne Route53
> Pfad zu, pub-key anpassen !!
```bash
#!/usr/bin/env bash
set -euo pipefail
...
Copy the ZIP folder to ec2 instace (home-dir)

```
scp -i .ssh/id_rsa Downloads/01_Flask-Python-E-Commerce-Website.zip ubuntu@34.192.144.129:~
```


```
mkdir -pv /opt/Flask-Python-E-Commerce-Website
sudo unzip -o ~/01_Flask-Python-E-Commerce-Website.zip -d opt/Flask-Python-E-Commerce-Website
```


Change the ownership to install the pip requirements
```
sudo chmod 770 /opt/Flask-Python-E-Commerce-Website
sudo chown -R ubuntu:ubuntu /opt/Flask-Python-E-Commerce-Website
```


Connection to database-server (back-end) is only over webserver possible. Therefor we have to make sure that the webserver's pub-key is copied on the database-server.

--> Choose the webserver keypair in aws webconsole and create the ec2 instance
